...
class ZAssetsUtils {
	...
	private static function getAssetsData(path:String, xmlid:String, project:ZProjectData, ifUnless:Bool = false):{
		...
		var files:Array<String> = [project.getAssetsRenamePath(StringUtils.getName(xmlid) + ".xml")];
		// 开始遍历所需资源
		for (file in builder.assetsLoads) {
			if (project.assetsPath.exists(file + ".png")
				...
				spines.push({
					...
					json: StringTools.replace(project.getAssetsRenamePath(file + ".json"), Sys.getCwd(), "")
				});
