...
class ZSceneManager {
    ...
    public function releaseScene(zScene:ZScene):Void
    {
        ...
        _sceneMaps.remove(cast Type.getClass(zScene));
...
class ZSceneManager {
    ...
    private var _sceneMaps:Dictionary<Class<ZScene>,ZScene>;

    public function new(){
        _sceneMaps = new Dictionary<Class<ZScene>,ZScene>();
    }
    ...
    public function createScene(cName:Class<ZScene>):ZScene
    {
        var scene:ZScene = _sceneMaps.get(cName);
        if(scene == null)
        {
            scene = Type.createInstance(cName,[]);
            _sceneMaps.set(cName,scene);
            Start.current.addChild(scene);
        }
        else{
            Start.current.addChild(scene);
    ...
    public function releaseSceneFormClass(cName:Class<ZScene>):Void
    {
        var classObj:Iterator<Class<ZScene>> = _sceneMaps.iterator();
        while(classObj.hasNext())
        {
            var c:Class<ZScene> = classObj.next();
            if(c == cName)
            {
                var zscene:ZScene = _sceneMaps.get(c);
                if(Std.is(zscene,cName))
                    releaseScene(zscene);
            }
    ...
    public function replaceSceneFormScene(scene:ZScene):ZScene
    {
        _sceneMaps.set(Type.getClass(scene),scene);
        Start.current.addChild(scene);
        _scenes.push(scene);
        return scene;
    }
