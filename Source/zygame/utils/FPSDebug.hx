...
class FPSDebug extends ZBox {
		...
		if (visible) {
			if (debugMsg != null)
				...
				if (memoryInfo != null) {
					...
					msg += "\nAMEN:" + Math.round(memoryInfo.availMem / 1024 / 1024 * 100) / 100 + "MB";
				}
				#end
	...
	/**
	 * 获取CPU消耗指数，200则接近100%使用率，持续高频的cpu会引起FPS下降
	 */
	public function getCpu():Int {
		return Std.int(_allcpu / _alltimes / 8 * 100);
	}
