...
class FPSDebug extends Sprite {
	...
	public function new(inX:Float = 0.0, inY:Float = 0.0, inCol:Int = 0xffffff) {
		...
		BitmapData.loadFromBase64(FPSAssets.assets, "image/png").onComplete(function(bitmapData:BitmapData):Void {
			...
			this.graphics.drawRect(inX, inY, _text.width * 2, _text.height);
			this.addChild(_text);
	...
	private function onEnter(_) {
		...
		if (visible) {
			...
			if (debugMsg != null)
				...
				_text.dataProvider = "MODE:" + Lib.getRenderMode() + "_MEM:" + mem + "MB_MaxMEN:" + memPeak + "MB_UPDATES:"
					+ zygame.core.Start.current.getUpdateLength() + "_SUPDATES:" + SpineManager.count() + "_FPS:" + getFps() + "_DrawCalls:"
					+ (_curDrawCall - 2) #if html5 + "_Texture:" + zygame.core.Start.TEXTURE_COUNT #end#if wechat + "_Context:" + untyped window.contextCount +
				"_Image:" + untyped window.imageCount #end;
		}
	}
...
class FPSAssets {
	...
	public static var fnt:String = '12<?xml version="1.0" encoding="UTF-8"?>
