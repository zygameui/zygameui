...
import zygame.display.batch.TileSpriteRender;
...
class Image extends DisplayObjectContainer {
	...
	private var _tilemap:TileSpriteRender;
	...
	private function set_bitmapData(bitmapData:Dynamic):Dynamic {
		...
		if (bitmapData == null) {
			...
			// 精灵表单渲染
			if (_tilemap == null) {
				_tilemap = new TileSpriteRender();
				_tilemap.smoothing = _smoothing;
	...
	private function updateScale9():Void {
		...
		// if (_tilemap.curFrame.scale9rect != null) {
		// 	// 默认支持九宫格
		// 	if (_scale9 == null) {
	...
	override public function alignPivot(v:Align = null, h:Align = null):Void {
		...
		if (_bitmap == null && _tilemap == null)
			return;
		if (isBitmapDataDraw)
			zygame.utils.Align.AlignTools.alignDisplay(_bitmap, v, h);
		else {
			zygame.utils.Align.AlignTools.alignDisplay(_tilemap, v, h);
		}
	}
